	...
	obj.hover(function(){
		...
		obj.click(function(){
			...
			$('.point')
				...
				.prepend($('<div class="flex-row row-border row-padding"><p>Дата образования:</p>'+'<p>'+paths[arr[this.id]].date+'</p></div>'))
				...
				.prepend($('<div class="flex-row row-border row-padding"><p>Экономический район:</p>'+'<p>'+paths[arr[this.id]].econom+'</p></div>'))
				.prepend($('<div class="flex-row row-border row-padding"><p>Федеральный округ:</p>'+'<p>'+paths[arr[this.id]].federal+'</p></div>'))
				.prepend($('<div class="flex-row row-border row-padding"><p>Столица:</p>'+'<p>'+paths[arr[this.id]].capital+'</p></div>'))
				
				
				
				
				.prepend($('<div class="containerInfo"> <img style="width:50px;" src="flags/'+arr[this.id]+'.png"> <p>'+paths[arr[this.id]].name+'</p> </div>'))
				.css({
					left: left + 400 > winWidth ? point.x - 200 : left,
					top: top < winHeight/2 && top+50,
